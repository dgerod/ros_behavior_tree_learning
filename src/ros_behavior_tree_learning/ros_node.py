...
class GpNode:
    ...
    def __init__(self, name, settings_file_path, parameters_file_path, outputs_directory_path):

        self._connector = Connector(name)
        self._controller = ControllerTask(settings_file_path, parameters_file_path, outputs_directory_path,
...
def start_node(name, bt_settings_path, gp_parameters_path, outputs_directory_path=""):

    rospy.init_node(name)

    try:
        node = GpNode(name, bt_settings_path, gp_parameters_path, outputs_directory_path)
        rospy.spin()

...
class GpNode:
    ...
    def __init__(self, name, settings_path):
        ...
        self._controller = ControllerTask(settings_path, self._connector.ports(), self._connector.publisher())
...
def start_node(name, settings_path):

    rospy.init_node(name)

    try:
        node = GpNode(name, settings_path)
        rospy.spin()
